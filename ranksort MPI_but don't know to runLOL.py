...
def parallel_ranksort(array, comm):
    ...
    if len(array) <= THRESHOLD:
        ...
        subarrays = [array[i:i+len(array)//size] for i in range(0, len(array), len(array)//size)]
...
if __name__ == '__main__':
    array_sizes = [ARRAY_SIZE_1, ARRAY_SIZE_2, ARRAY_SIZE_3, ARRAY_SIZE_4, ARRAY_SIZE_5]
    num_nodes_list = [1, 4, 8, 12, 16]
    comm = MPI.COMM_WORLD
    for array_size in array_sizes:
        ...
        for num_nodes in num_nodes_list:
            ...
            if comm.Get_rank() == 0:
                ...
                end_time = time.time()
                print(f"Execution time for {num_nodes} nodes and {array_size} elements: {end_time - start_time} seconds")
